...
export function BaseTable<TData, TValue>({
  ...
  return (
    <div className="w-full rounded-md border border-slate-200 dark:border-slate-700">
      {showToolbar && (
        <DataTableToolbar
          table={table}
          {...toolbarProps}
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-40 py-40">
                  <div className="flex items-center justify-center">
                    <Spinner className="w-20 border-black" />
                  </div>
                </TableCell>
              </TableRow>
            )}
...
export function DataTableColumnHeader<TData, TValue>({
  ...
  if (!column.getCanSort()) {
    return (
      <div className={cn("text-[16px] font-bold leading-none", className)}>
        {title}
      </div>
    );
  }
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            className={cn(
              "focus:outline-none px-2 -ml-2 h-8 data-[state=open]:bg-gray/10",
              "flex items-center justify-center rounded hover:bg-gray/10",
